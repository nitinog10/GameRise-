...
export default function IntegrationSettings(){
  ...
  const [form,setForm]=useState({gameSlug:'valorant',apiKey:'',region:'ap',inGameUsername:'',accountId:'',isActive:true});
  ...
  return <div className='min-h-screen bg-[#1a1a24] text-white'><Navigation/><div className='max-w-5xl mx-auto pt-24 px-4 pb-10'>
    ...
    <div className='bg-black/20 p-4 rounded mb-4 space-y-2'>
      ...
      {['gameSlug','apiKey','region','inGameUsername','accountId'].map(k=><input key={k} className='input w-full' placeholder={k} value={form[k]} onChange={e=>setForm({...form,[k]:e.target.value})}/>)}
      <button className='btn-primary' onClick={async()=>{await api.post('/api/observer/integrations',form); load();}}>Link Account</button>
    </div>
    <div className='grid gap-2 mb-6'>{integrations.map(i=><div key={i.integrationId} className='bg-black/20 rounded p-3 flex justify-between'><span>{i.gameName||i.gameSlug||i.game} • {i.inGameUsername||i.accountId}</span><span>{i.status||'pending'} {i.lastSyncAt?`• ${new Date(i.lastSyncAt).toLocaleString()}`:'• Syncing'}</span></div>)}</div>
