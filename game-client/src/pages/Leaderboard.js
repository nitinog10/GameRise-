...
const Leaderboard = () => {
  ...
  const matchesForBoard = useMemo(
    () => (gameFilter ? matches.filter((m) => m.gameSlug === gameFilter) : matches),
    [gameFilter, matches]
  );
      ...
      <div className="max-w-7xl mx-auto pt-24 px-4 pb-10">
        ...
        {board.length === 0 && !loading ? (
          <div className="glass-card rounded-xl p-6 text-gray-400 text-sm">
            No match data yet. Play a match or log one in the main dashboard to populate the leaderboard.
          </div>
        ) : (
          <>
            {/* Top 3 podium */}
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-right hidden xl:table-cell">
                        <div className="flex justify-end gap-1">
                          {pl.games.map((g) => (
                            <span key={g} className="text-sm" title={g}>
                              {resolveGameMeta(gameMeta, g).icon}
                            </span>
                          ))}
                        </div>
                      </td>
